...
import { getDatesInRange, getRemark } from '../controller/attendance.js'
...
//Expor attendace by the attendance status (present and absent)
    ...
    try {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            for (let j = 1; j <= total_days; j++) {
                ...
                const attendance = await AttendanceStatus.findOne({
                    $and:
                        [
                            { emp_id: results[i].emp_id },
                ...
                emp[start_date] = attendance ? attendance.response_status === 0 ? 'Present' : 'Absent' : 'Not Marked'
            }
            employees.push(emp)
        }
    ...
    try {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            for (let j = 0; j < start_date.length; j++) {
                ...
                emp[start_date[j]] = attendance ? getAttendanceStatus(attendance.response_status) : 'Not Marked'
            }
            employees.push(emp)
        }
